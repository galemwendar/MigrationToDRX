...
[Obsolete("Используйте MainPage. Этот компонент будет удален в будущих версиях.")]
public partial class ObsoletteMainPage
{
    #region Сервисы
    /// <summary>
    ...
    /// Сценарий работы
    /// </summary>
    private OdataOperation CurrentScenario = OdataOperation.CreateEntity;

    /// <summary>
    /// Модель сценария работы
    /// </summary>
    /// <param name="Value"></param>
    /// <param name="Description"></param>
    private record ScenarioOption(OdataOperation Value, string Description);

    /// <summary>
    /// Список доступных для выбора сценариев
    /// </summary>
    private List<OdataOperation> ScenarioOptions = new()
    {
    };

    /// <summary>
    /// Коллекция свойств сущности Odata
    /// </summary>
    private List<EntityFieldDto> BaseEntityFieldsToMap = new();

    /// <summary>
    /// Коллекция свойств сущности Odata + допсвойства
    /// </summary>
    ...
    {
        CurrentScenario = (OdataOperation)value;
        RecalculateEntityFields();
        RefreshAvailableFields();
        StateHasChanged();
    }
